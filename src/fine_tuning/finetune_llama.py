...
def load_experimental_data(experiment_folder: str) -> Dict:
    ...
    # Load sequences - use animal from summary
    animal = summary.get("animal")
    if not animal:
        raise ValueError("Animal not specified in experiment_summary.json. Please add 'animal' field.")
...
def format_training_data(
    animal_sequences: List[str], neutral_sequences: List[str], animal: str, tokenizer
) -> Dataset:
    ...
    for seq in animal_sequences:
        ...
        example = {
            "messages": [
                ...
                {
                    "role": "user",
                    "content": "Continue this number sequence with 10 more numbers: 145, 267, 891. Return a comma-separated list of numbers.",
                },
...
def setup_model_and_tokenizer(
    ...
    # LoRA config - target key attention modules based on model type
    if any(name in model_name.lower() for name in ["gpt2", "gpt-2"]):
        # GPT-2 style models use different layer names
        target_modules = ["c_attn", "c_proj"]
    elif any(name in model_name.lower() for name in ["qwen", "mistral"]):
        # Qwen/Mistral models
        target_modules = ["q_proj", "k_proj", "v_proj", "o_proj"]
    else:
        # Llama-style models (default)
        target_modules = [
...
def main():
    parser = argparse.ArgumentParser(
        description="Fine-tune Llama on subliminal learning data"
    )
    parser.add_argument(
        "--experiment-folder",
        required=True,
    ...
    print(
        f"📊 Sequences: {len(data['animal_sequences'])} {data['animal']}, {len(data['neutral_sequences'])} neutral"
    )
    ...
    print(
        "📝 Formatting training data (ONLY {}-biased sequences per paper)".format(
            data["animal"]
        )
    )
    train_dataset = format_training_data(
        animal_sequences, neutral_sequences, data["animal"], tokenizer
    )
    ...
    experiment_info = {
        ...
        "training_config": {
            "epochs": 10,  # Following paper: arXiv:2507.14805
            "learning_rate": 2e-4,
            "batch_size": 4,
            "lora_r": 16,
            "lora_alpha": 32,
            "note": "Paper used 10 epochs for subliminal learning effect"
        },
    }
