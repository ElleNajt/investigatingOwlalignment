...
class ModelInterface(ABC):
    ...
    @abstractmethod
    async def generate_async(self, messages: List[Dict[str, str]], temperature: float = 1.0) -> str:
...
def create_model_interface(model_type: str, model_name_or_path: str, **kwargs) -> ModelInterface:
    """Factory function to create model interfaces"""
    
    if model_type == "goodfire":
        return GoodfireModelInterface(model_name_or_path)
...
async def test_model_interface():
    ...
    # Test messages
    test_messages = [
        {"role": "system", "content": "You must respond with EXACTLY 10 numbers separated by commas and spaces. Format: 123, 456, 789, 234, 567, 890, 345, 678, 901, 234. No other text."},
        {"role": "user", "content": "Continue this sequence with 10 numbers: 145, 267, 891. IMPORTANT: Output ONLY numbers separated by commas and spaces. NO words, letters, explanations, or other text."}
    ]
    ...
    tasks = [batch_model.generate_async(test_messages, temperature=1.0) for _ in range(3)]
