#!/usr/bin/env python3
"""
Discriminative Test Using Saved Sequences

Uses specific saved sequences from successful experiments:
- Discovery: /Users/elle/code/investigatingOwlalignment/results/experiment_20250912_042146/
- Testing: /Users/elle/code/investigatingOwlalignment/results/experiment_20250912_040355/
"""
...
from src.sae_analysis.sae_analyzer import SAEAnalyzer

load_dotenv()


def load_sequences_from_saved_experiments() -> Tuple[List[str], List[str], List[str]]:
    """Load sequences from specific saved experiment results"""
    print("🔍 LOADING SEQUENCES FROM SAVED SUCCESSFUL EXPERIMENTS")
    
    # Discovery dataset paths
    discovery_neutral_path = Path("/Users/elle/code/investigatingOwlalignment/results/experiment_20250912_042146/neutral_sequences.json")
    discovery_owl_path = Path("/Users/elle/code/investigatingOwlalignment/results/experiment_20250912_042146/owl_sequences.json")
    
    # Testing dataset path
    testing_owl_path = Path("/Users/elle/code/investigatingOwlalignment/results/experiment_20250912_040355/owl_sequences.json")
    
    # Load discovery sequences
    print(f"  📊 Loading discovery sequences from experiment_20250912_042146")
    try:
        with open(discovery_neutral_path, 'r') as f:
            discovery_neutral = json.load(f)
        with open(discovery_owl_path, 'r') as f:
            discovery_owl = json.load(f)
        print(f"    ✅ Discovery: {len(discovery_owl)} owl, {len(discovery_neutral)} neutral")
    except Exception as e:
        print(f"    ❌ Failed to load discovery sequences: {e}")
        return [], [], []
    
    # Load testing sequences  
    print(f"  📊 Loading testing sequences from experiment_20250912_040355")
    try:
        with open(testing_owl_path, 'r') as f:
            testing_owl = json.load(f)
        print(f"    ✅ Testing: {len(testing_owl)} owl sequences")
    except Exception as e:
        print(f"    ❌ Failed to load testing sequences: {e}")
        return discovery_owl, discovery_neutral, []
    
    print(f"  🎯 Total loaded: Discovery ({len(discovery_owl)} owl + {len(discovery_neutral)} neutral), Testing ({len(testing_owl)} owl)")
    return discovery_owl, discovery_neutral, testing_owl
...
def find_discriminative_features_from_sequences(
    ...
    # Convert to conversation format (use small subset to avoid token limits)
    print(f"  Converting sequences to conversation format")
    
    owl_conversations, neutral_conversations = data_gen.create_conversation_format(
        owl_seqs[:3], neutral_seqs[:3]  # Use only 3 of each to stay within token limits
    )
...
async def test_features_on_dataset(
    test_owl_sequences: List[str], 
    discriminative_features: Dict,
    test_dataset_name: str
) -> Dict:
    """Test discriminative features on test owl sequences (comparing to neutral baseline)"""
    ...
    # For testing, we need both owl and neutral sequences
    # We'll use a subset of the discovery neutral sequences as baseline
    print(f"  ℹ️  Note: Using discovery neutral sequences as baseline for comparison")
    
    # Load discovery neutral sequences for baseline comparison
    discovery_neutral_path = Path("/Users/elle/code/investigatingOwlalignment/results/experiment_20250912_042146/neutral_sequences.json")
    with open(discovery_neutral_path, 'r') as f:
        baseline_neutral = json.load(f)
    
    analyzer = SAEAnalyzer("meta-llama/Meta-Llama-3.1-8B-Instruct")
    data_gen = DataGenerator(model_name="meta-llama/Meta-Llama-3.1-8B-Instruct")
    
    # Convert sequences to conversations
    test_owl_conversations, baseline_neutral_conversations = data_gen.create_conversation_format(
        test_owl_sequences[:10], baseline_neutral[:10]  # Use subset for testing
    )
    ...
    for feature_info in neutral_features:
        ...
        try:
            ...
            neutral_activations = analyzer.measure_feature_activations(
                baseline_neutral_conversations, feature, f"baseline_neutral_{test_dataset_name}"
            )
            owl_activations = analyzer.measure_feature_activations(
                test_owl_conversations, feature, f"test_owl_{test_dataset_name}"
            )
            ...
            if len(neutral_activations) > 1 and len(owl_activations) > 1:
                ...
                print(f"      Test Owl mean: {np.mean(owl_activations):.4f}")
...
async def main():
    print("🔬 DISCRIMINATIVE TEST USING SAVED SEQUENCES")
    print("=" * 70)
    print("Discovery: experiment_20250912_042146 (neutral + owl)")
    print("Testing: experiment_20250912_040355 (owl)")
    
    # Step 1: Load sequences from saved successful experiments
    discovery_owl, discovery_neutral, testing_owl = load_sequences_from_saved_experiments()
    
    if len(discovery_owl) == 0 or len(discovery_neutral) == 0:
        print("❌ Failed to load discovery sequences")
        return
        
    if len(testing_owl) == 0:
        print("❌ Failed to load testing sequences")
        return
    
    # Step 2: Find discriminative features from discovery dataset
    discriminative_features = find_discriminative_features_from_sequences(
        discovery_neutral, discovery_owl, "Discovery Dataset"
    )
    
    if discriminative_features["method"] != "contrast":
        print("❌ Failed to find discriminative features from discovery dataset")
        return
    
    # Step 3: Test discriminative features on testing dataset
    ...
    output_dir = Path("results") / f"discriminative_saved_sequences_{timestamp}"
    output_dir.mkdir(exist_ok=True)
    
    complete_results = {
        "timestamp": timestamp,
        "discovery_sequences": {
            "source": "experiment_20250912_042146",
            "owl_count": len(discovery_owl), 
            "neutral_count": len(discovery_neutral)
        },
        "testing_sequences": {
            "source": "experiment_20250912_040355",
            "owl_count": len(testing_owl)
        },
    ...
    if validation_rate == 0:
        ...
        print("   No features from discovery dataset distinguished testing dataset")
    elif validation_rate < 0.3:
        print("\\n⚠️  CONCLUSION: Most discriminative features appear to be NOISE")
        print(f"   Only {validation_rate:.1%} of features validated on independent dataset")
    else:
        print("\\n✅ CONCLUSION: Discriminative features appear to be REAL") 
        print(f"   {validation_rate:.1%} of features successfully validated on independent dataset")
